...
## Summary
This spoke defines when market studies can support behavior claims and how to avoid overstating them.
...
Primary working inputs: study URL, sample scope, publication date, claim wording.
...
## Workflow
- Define the decision this note supports: defines when market studies can support behavior claims and how to avoid overstating them.
...
## Risk Controls
- Evidence currency risk: Primary Study Source Rules decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Primary Study Source Rules advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Primary Study Source Rules into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Primary Study Source Rules guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Primary Study Source Rules outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Primary Study Source Rules decision summary for the editor or auditor.
...
- A blocked-claims list for Primary Study Source Rules gaps that need more evidence.
...
- A no-action statement when Primary Study Source Rules evidence is incomplete or outside this brain.
