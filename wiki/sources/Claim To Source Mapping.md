...
## Summary
This spoke links each operational claim to the exact dated source that supports it.
...
Primary working inputs: claim inventory, source URL, retrieved date, confidence.
...
## Workflow
- Define the decision this note supports: links each operational claim to the exact dated source that supports it.
...
## Risk Controls
- Evidence currency risk: Claim To Source Mapping decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Claim To Source Mapping advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Claim To Source Mapping into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Claim To Source Mapping guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Claim To Source Mapping outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Claim To Source Mapping decision summary for the editor or auditor.
...
- A blocked-claims list for Claim To Source Mapping gaps that need more evidence.
...
- A no-action statement when Claim To Source Mapping evidence is incomplete or outside this brain.
