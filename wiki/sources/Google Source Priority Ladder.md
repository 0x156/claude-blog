...
## Summary
This spoke orders Google-owned sources by authority for ranking, schema, AI features, APIs, and policy claims.
...
Primary working inputs: source type, URL, topic, freshness date.
...
## Workflow
- Define the decision this note supports: orders Google-owned sources by authority for ranking, schema, AI features, APIs, and policy claims.
...
## Risk Controls
- Evidence currency risk: Google Source Priority Ladder decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Google Source Priority Ladder advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Google Source Priority Ladder into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Google Source Priority Ladder guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Google Source Priority Ladder outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Google Source Priority Ladder decision summary for the editor or auditor.
...
- A blocked-claims list for Google Source Priority Ladder gaps that need more evidence.
...
- A no-action statement when Google Source Priority Ladder evidence is incomplete or outside this brain.
