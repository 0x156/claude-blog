...
## Summary
This spoke sets monthly, release-time, and on-change refresh routines for the source pack.
...
Primary working inputs: source group, owner, due date, trigger.
...
## Workflow
- Define the decision this note supports: sets monthly, release-time, and on-change refresh routines for the source pack.
...
## Risk Controls
- Evidence currency risk: Source Refresh Cadence decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Source Refresh Cadence advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Source Refresh Cadence into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Source Refresh Cadence guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Source Refresh Cadence outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Source Refresh Cadence decision summary for the editor or auditor.
...
- A blocked-claims list for Source Refresh Cadence gaps that need more evidence.
...
- A no-action statement when Source Refresh Cadence evidence is incomplete or outside this brain.
