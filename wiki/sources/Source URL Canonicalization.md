...
## Summary
This spoke keeps source URLs stable, exact, and deduplicated across source_urls frontmatter and body citations.
...
Primary working inputs: URL list, canonical URL, redirects, duplicates.
...
## Workflow
- Define the decision this note supports: keeps source URLs stable, exact, and deduplicated across source_urls frontmatter and body citations.
...
## Risk Controls
- Evidence currency risk: Source URL Canonicalization decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Source URL Canonicalization advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Source URL Canonicalization into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Source URL Canonicalization guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Source URL Canonicalization outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Source URL Canonicalization decision summary for the editor or auditor.
...
- A blocked-claims list for Source URL Canonicalization gaps that need more evidence.
...
- A no-action statement when Source URL Canonicalization evidence is incomplete or outside this brain.
