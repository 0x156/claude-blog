...
## Summary
Heading Hierarchy Rules is the spoke for define H1, H2, H3, and answer block roles for a draftable outline.
...
The durable output is a heading hierarchy rule set.
The success condition is that each heading has one job and makes extraction easier for readers and AI systems.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Heading Hierarchy Rules.
...
- Metric set for review: heading clarity, answer-first coverage, scanability, and passage length.

## Operating Procedure
- Start with the precise decision that Heading Hierarchy Rules must make.
...
- Reject the failure pattern of building decorative headings that do not answer the query.
...
## Acceptance Criteria
- The heading hierarchy rule set names the exact object under review.
...
- The note accepts building decorative headings that do not answer the query.
...
- The next review should verify whether heading clarity, answer-first coverage, scanability, and passage length still captures the decision quality.
