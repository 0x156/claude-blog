...
## Summary
Brief Source Pack is the spoke for assemble official, primary, vendor, and practitioner sources for a brief.
...
The durable output is a brief source pack.
The success condition is that writers can trace every current claim to a real source URL.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Brief Source Pack.
...
- Metric set for review: source type mix, retrieval dates, claim mapping, and refresh needs.

## Operating Procedure
- Start with the precise decision that Brief Source Pack must make.
...
- Reject the failure pattern of using search snippets or AI summaries as source evidence.
...
## Acceptance Criteria
- The brief source pack names the exact object under review.
...
- The note accepts using search snippets or AI summaries as source evidence.
...
- The next review should verify whether source type mix, retrieval dates, claim mapping, and refresh needs still captures the decision quality.
