...
## Summary
Reader Job Statement is the spoke for turn query intent into a specific reader job, success condition, and decision context.
...
The durable output is a reader job statement.
The success condition is that the outline answers a real job rather than a keyword phrase.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Reader Job Statement.
...
- Metric set for review: job clarity, decision stage, objections, and required proof.

## Operating Procedure
- Start with the precise decision that Reader Job Statement must make.
...
- Reject the failure pattern of writing for search volume while ignoring the reader decision.
...
## Acceptance Criteria
- The reader job statement names the exact object under review.
...
- The note accepts writing for search volume while ignoring the reader decision.
...
- The next review should verify whether job clarity, decision stage, objections, and required proof still captures the decision quality.
