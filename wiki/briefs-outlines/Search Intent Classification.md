...
## Summary
Search Intent Classification is the spoke for classify the primary and secondary search intent behind a query set.
...
The durable output is a intent classification note.
The success condition is that briefs align structure and answer depth to the dominant reader task.
...
- Sibling comparison starts with [[SERP Brief Input Contract]] and [[Reader Job Statement]].
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Search Intent Classification.
...
- Metric set for review: intent confidence, SERP pattern notes, mismatch risks, and content angle fit.

## Operating Procedure
- Start with the precise decision that Search Intent Classification must make.
...
- Reject the failure pattern of copying competitor formats without deciding the reader task.
...
## Acceptance Criteria
- The intent classification note names the exact object under review.
...
- The note accepts copying competitor formats without deciding the reader task.
...
- The next review should verify whether intent confidence, SERP pattern notes, mismatch risks, and content angle fit still captures the decision quality.
