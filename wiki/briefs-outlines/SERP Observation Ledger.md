...
## Summary
SERP Observation Ledger is the spoke for record dated SERP observations that informed the brief without treating them as ranking facts.
...
The durable output is a SERP observation ledger.
The success condition is that SERP snapshots guide format choices while staying separate from source truth.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs SERP Observation Ledger.
...
- Metric set for review: observation date, locale, device, feature set, and confidence.

## Operating Procedure
- Start with the precise decision that SERP Observation Ledger must make.
...
- Reject the failure pattern of using a single live SERP as proof of a ranking factor.
...
## Acceptance Criteria
- The SERP observation ledger names the exact object under review.
...
- The note accepts using a single live SERP as proof of a ranking factor.
...
- The next review should verify whether observation date, locale, device, feature set, and confidence still captures the decision quality.
