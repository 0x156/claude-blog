...
## Summary
This spoke chooses examples that match the reader, source evidence, risk level, and locale.
...
Primary working inputs: example list, source dates, persona fit, local constraints.
...
## Workflow
- Define the decision this note supports: chooses examples that match the reader, source evidence, risk level, and locale.
...
## Risk Controls
- Evidence currency risk: Example Selection Rules decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Example Selection Rules advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Example Selection Rules into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Example Selection Rules guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Example Selection Rules outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Example Selection Rules decision summary for the editor or auditor.
...
- A blocked-claims list for Example Selection Rules gaps that need more evidence.
...
- A no-action statement when Example Selection Rules evidence is incomplete or outside this brain.
