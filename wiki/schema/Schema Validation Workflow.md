...
## Summary
Schema Validation Workflow is a schema review note for blog schema stack.
It validates syntax, vocabulary, Google support, and content alignment before handoff.
Use it with [[Blog Schema Stack]] when the working unit is a blog post, entity graph, or structured data block.

## Operating Question
- What validation evidence proves the schema is technically and editorially aligned?
- The expected output is a validation workflow with tool outputs and manual checks.
- The main risk is passing syntax while failing visible-content alignment.
...
## Specific Checks
- Check that schema validation workflow is applied to the correct content unit.
- Check that the decision matches this purpose: validates syntax, vocabulary, Google support, and content alignment before handoff.
- Check that the output can be inspected as a validation workflow with tool outputs and manual checks.
- Check that the risk is addressed directly: passing syntax while failing visible-content alignment.
