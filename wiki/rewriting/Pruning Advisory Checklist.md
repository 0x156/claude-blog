...
## Summary
Pruning Advisory Checklist is a rewrite decision note for freshness and decay.
It sets a high bar before recommending removal, noindex, or depublication.
Use it with [[Freshness and Content Decay]] when the working unit is a published post, cluster, or decayed section.

## Operating Question
- Is pruning safer and more useful than refreshing, merging, or leaving the page live?
- The expected output is a pruning checklist with evidence, risk, and approval need.
- The main risk is treating low traffic as automatic proof that a page has no value.
...
## Specific Checks
- Check that pruning advisory checklist is applied to the correct content unit.
- Check that the decision matches this purpose: sets a high bar before recommending removal, noindex, or depublication.
- Check that the output can be inspected as a pruning checklist with evidence, risk, and approval need.
- Check that the risk is addressed directly: treating low traffic as automatic proof that a page has no value.
