...
## Summary
Update Timestamp Policy is a rewrite decision note for freshness and decay.
It defines when visible published, modified, and reviewed dates should change.
Use it with [[Freshness and Content Decay]] when the working unit is a published post, cluster, or decayed section.

## Operating Question
- What visible date change is justified by the actual editorial work?
- The expected output is a timestamp recommendation with source and review evidence.
- The main risk is changing dates cosmetically without meaningful content review.
...
## Specific Checks
- Check that update timestamp policy is applied to the correct content unit.
- Check that the decision matches this purpose: defines when visible published, modified, and reviewed dates should change.
- Check that the output can be inspected as a timestamp recommendation with source and review evidence.
- Check that the risk is addressed directly: changing dates cosmetically without meaningful content review.
