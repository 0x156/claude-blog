...
## Summary
Rewrite Rollback Notes is a rewrite decision note for freshness and decay.
It records how to reverse or adjust a rewrite if performance or quality signals degrade.
Use it with [[Freshness and Content Decay]] when the working unit is a published post, cluster, or decayed section.

## Operating Question
- What should be restored or changed if the rewrite underperforms?
- The expected output is a rollback note with triggers, preserved content, and review date.
- The main risk is making irreversible editorial changes without a safety path.
...
## Specific Checks
- Check that rewrite rollback notes is applied to the correct content unit.
- Check that the decision matches this purpose: records how to reverse or adjust a rewrite if performance or quality signals degrade.
- Check that the output can be inspected as a rollback note with triggers, preserved content, and review date.
- Check that the risk is addressed directly: making irreversible editorial changes without a safety path.
