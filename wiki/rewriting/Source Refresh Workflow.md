...
## Summary
Source Refresh Workflow is a rewrite decision note for freshness and decay.
It updates dated claims while preserving provenance and current requirements.
Use it with [[Freshness and Content Decay]] when the working unit is a published post, cluster, or decayed section.

## Operating Question
- Which sources are stale, superseded, or missing for this page?
- The expected output is a source refresh log with replaced and retained URLs.
- The main risk is refreshing prose while leaving old evidence in place.
...
## Specific Checks
- Check that source refresh workflow is applied to the correct content unit.
- Check that the decision matches this purpose: updates dated claims while preserving provenance and current requirements.
- Check that the output can be inspected as a source refresh log with replaced and retained URLs.
- Check that the risk is addressed directly: refreshing prose while leaving old evidence in place.
