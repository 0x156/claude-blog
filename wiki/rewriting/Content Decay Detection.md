...
## Summary
Content Decay Detection is a rewrite decision note for freshness and decay.
It finds whether performance loss comes from traffic decay, stale claims, intent drift, or trust gaps.
Use it with [[Freshness and Content Decay]] when the working unit is a published post, cluster, or decayed section.

## Operating Question
- What evidence shows that this post is decaying rather than merely fluctuating?
- The expected output is a decay diagnosis with data source, date range, and confidence.
- The main risk is rewriting a page because of one noisy metric.
...
## Specific Checks
- Check that content decay detection is applied to the correct content unit.
- Check that the decision matches this purpose: finds whether performance loss comes from traffic decay, stale claims, intent drift, or trust gaps.
- Check that the output can be inspected as a decay diagnosis with data source, date range, and confidence.
- Check that the risk is addressed directly: rewriting a page because of one noisy metric.
...
- Use [[Refresh Versus Rewrite Decision]] when this note needs a sibling follow-up.
- Use [[Source Refresh Workflow]] when the next decision belongs beside this note.
