...
## Summary
Intent Drift Audit is a rewrite decision note for freshness and decay.
It checks whether the SERP and reader need have moved away from the original angle.
Use it with [[Freshness and Content Decay]] when the working unit is a published post, cluster, or decayed section.

## Operating Question
- Does the article still match the intent that now dominates the topic?
- The expected output is an intent-drift note with SERP observations and rewrite options.
- The main risk is blaming algorithm change when the page no longer answers the query.
...
## Specific Checks
- Check that intent drift audit is applied to the correct content unit.
- Check that the decision matches this purpose: checks whether the SERP and reader need have moved away from the original angle.
- Check that the output can be inspected as an intent-drift note with SERP observations and rewrite options.
- Check that the risk is addressed directly: blaming algorithm change when the page no longer answers the query.
