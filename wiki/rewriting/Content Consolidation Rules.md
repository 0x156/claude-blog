...
## Summary
Content Consolidation Rules is a rewrite decision note for freshness and decay.
It decides when overlapping posts should be merged or redirected in the editorial plan.
Use it with [[Freshness and Content Decay]] when the working unit is a published post, cluster, or decayed section.

## Operating Question
- Do multiple pages compete for the same reader task or query cluster?
- The expected output is a consolidation recommendation with retained URL and link updates.
- The main risk is merging pages without preserving the strongest evidence and intent.
...
## Specific Checks
- Check that content consolidation rules is applied to the correct content unit.
- Check that the decision matches this purpose: decides when overlapping posts should be merged or redirected in the editorial plan.
- Check that the output can be inspected as a consolidation recommendation with retained URL and link updates.
- Check that the risk is addressed directly: merging pages without preserving the strongest evidence and intent.
