...
## Summary
Channel Asset Inventory is the spoke for track which derivative assets exist, who owns them, and whether source fidelity was checked.
...
The durable output is a channel asset inventory.
The success condition is that every repurposed asset has a state, owner, canonical link, and review outcome.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Channel Asset Inventory.
...
- Metric set for review: asset count, missing owner count, channel readiness, and review status.

## Operating Procedure
- Start with the precise decision that Channel Asset Inventory must make.
...
- Reject the failure pattern of publishing variants without a source fidelity audit.
...
## Acceptance Criteria
- The channel asset inventory names the exact object under review.
...
- The note accepts publishing variants without a source fidelity audit.
...
- The next review should verify whether asset count, missing owner count, channel readiness, and review status still captures the decision quality.
