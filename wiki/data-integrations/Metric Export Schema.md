...
## Summary
This spoke describes the columns needed when metric exports are provided to the brain.
...
Primary working inputs: date, page, query, clicks, impressions, CTR, position, engagement fields.
...
## Workflow
- Define the decision this note supports: describes the columns needed when metric exports are provided to the brain.
...
## Risk Controls
- Evidence currency risk: Metric Export Schema decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Metric Export Schema advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Metric Export Schema into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Metric Export Schema guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Metric Export Schema outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Metric Export Schema decision summary for the editor or auditor.
...
- A blocked-claims list for Metric Export Schema gaps that need more evidence.
...
- A no-action statement when Metric Export Schema evidence is incomplete or outside this brain.
