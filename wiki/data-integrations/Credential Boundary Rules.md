...
## Summary
This spoke keeps API credentials outside notes and limits this brain to read-only recommendations.
...
Primary working inputs: access owner, permission scope, storage policy, audit notes.
...
## Workflow
- Define the decision this note supports: keeps API credentials outside notes and limits this brain to read-only recommendations.
...
## Risk Controls
- Evidence currency risk: Credential Boundary Rules decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Credential Boundary Rules advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Credential Boundary Rules into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Credential Boundary Rules guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Credential Boundary Rules outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Credential Boundary Rules decision summary for the editor or auditor.
...
- A blocked-claims list for Credential Boundary Rules gaps that need more evidence.
...
- A no-action statement when Credential Boundary Rules evidence is incomplete or outside this brain.
