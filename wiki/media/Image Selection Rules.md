...
## Summary
This spoke selects images that answer, demonstrate, compare, or prove instead of decorating weak sections.
...
Primary working inputs: draft, image brief, rights status, caption source.
...
## Workflow
- Define the decision this note supports: selects images that answer, demonstrate, compare, or prove instead of decorating weak sections.
...
## Risk Controls
- Evidence currency risk: Image Selection Rules decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Image Selection Rules advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Image Selection Rules into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Image Selection Rules guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Image Selection Rules outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Image Selection Rules decision summary for the editor or auditor.
...
- A blocked-claims list for Image Selection Rules gaps that need more evidence.
...
- A no-action statement when Image Selection Rules evidence is incomplete or outside this brain.
