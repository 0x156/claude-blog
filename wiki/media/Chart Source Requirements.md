...
## Summary
This spoke requires every chart to carry dated data, methodology, and source provenance.
...
Primary working inputs: dataset, chart claim, source URL, retrieval date.
...
## Workflow
- Define the decision this note supports: requires every chart to carry dated data, methodology, and source provenance.
...
## Risk Controls
- Evidence currency risk: Chart Source Requirements decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Chart Source Requirements advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Chart Source Requirements into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Chart Source Requirements guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Chart Source Requirements outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Chart Source Requirements decision summary for the editor or auditor.
...
- A blocked-claims list for Chart Source Requirements gaps that need more evidence.
...
- A no-action statement when Chart Source Requirements evidence is incomplete or outside this brain.
