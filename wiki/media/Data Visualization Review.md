...
## Summary
This spoke checks whether charts encode the right comparison and avoid misleading scale, labels, or cherry-picked windows.
...
Primary working inputs: chart draft, source table, axis choices, annotation notes.
...
## Workflow
- Define the decision this note supports: checks whether charts encode the right comparison and avoid misleading scale, labels, or cherry-picked windows.
...
## Risk Controls
- Evidence currency risk: Data Visualization Review decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Data Visualization Review advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Data Visualization Review into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Data Visualization Review guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Data Visualization Review outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Data Visualization Review decision summary for the editor or auditor.
...
- A blocked-claims list for Data Visualization Review gaps that need more evidence.
...
- A no-action statement when Data Visualization Review evidence is incomplete or outside this brain.
