...
## Summary
This spoke checks every visual claim against the same evidence rules as text claims.
...
Primary working inputs: visual asset, claim text, source URL, reviewer signoff.
...
## Workflow
- Define the decision this note supports: checks every visual claim against the same evidence rules as text claims.
...
## Risk Controls
- Evidence currency risk: Visual Claim Review decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Visual Claim Review advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Visual Claim Review into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Visual Claim Review guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Visual Claim Review outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Visual Claim Review decision summary for the editor or auditor.
...
- A blocked-claims list for Visual Claim Review gaps that need more evidence.
...
- A no-action statement when Visual Claim Review evidence is incomplete or outside this brain.
