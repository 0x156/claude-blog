...
## Summary
This spoke keeps audio summaries faithful to the article and clearly bounded as repurposed content.
...
Primary working inputs: article draft, audio script, claim list, channel plan.
...
## Workflow
- Define the decision this note supports: keeps audio summaries faithful to the article and clearly bounded as repurposed content.
...
## Risk Controls
- Evidence currency risk: Audio Summary Rules decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Audio Summary Rules advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Audio Summary Rules into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Audio Summary Rules guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Audio Summary Rules outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Audio Summary Rules decision summary for the editor or auditor.
...
- A blocked-claims list for Audio Summary Rules gaps that need more evidence.
...
- A no-action statement when Audio Summary Rules evidence is incomplete or outside this brain.
