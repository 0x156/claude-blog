...
## Summary
FLOW Factcheck Stage is the spoke for verify current claims, statistics, citations, and sensitive statements before delivery.
Use it when [[FLOW Framework]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a FLOW factcheck packet.
The success condition is that current claims are checked against dated sources.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs FLOW Factcheck Stage.
...
- Metric set for review: verified claims, unsupported claims, source age, and escalation count.

## Operating Procedure
- Start with the precise decision that FLOW Factcheck Stage must make.
...
- Reject the failure pattern of factchecking only the most visible statistics.
...
## Acceptance Criteria
- The FLOW factcheck packet names the exact object under review.
...
- The note accepts factchecking only the most visible statistics.
...
- The next review should verify whether verified claims, unsupported claims, source age, and escalation count still captures the decision quality.
