...
## Summary
FLOW Rewrite Stage is the spoke for handle refresh, consolidation, expansion, or pruning work with source and rollback discipline.
Use it when [[FLOW Framework]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a FLOW rewrite packet.
The success condition is that rewrites improve usefulness without hiding risk or changing unsupported claims.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs FLOW Rewrite Stage.
...
- Metric set for review: changed sections, source updates, rollback notes, and refresh trigger.

## Operating Procedure
- Start with the precise decision that FLOW Rewrite Stage must make.
...
- Reject the failure pattern of rewriting stale content without preserving source history.
...
## Acceptance Criteria
- The FLOW rewrite packet names the exact object under review.
...
- The note accepts rewriting stale content without preserving source history.
...
- The next review should verify whether changed sections, source updates, rollback notes, and refresh trigger still captures the decision quality.
