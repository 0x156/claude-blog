...
## Summary
FLOW Confidence Tags is the spoke for assign verified or advisory confidence to recommendations and source-backed claims.
Use it when [[FLOW Framework]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a FLOW confidence tag guide.
The success condition is that readers can tell fact from recommendation.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs FLOW Confidence Tags.
...
- Metric set for review: verified count, advisory count, unknown count, and evidence gap.

## Operating Procedure
- Start with the precise decision that FLOW Confidence Tags must make.
...
- Reject the failure pattern of marking a workflow suggestion as verified because it is useful.
...
## Acceptance Criteria
- The FLOW confidence tag guide names the exact object under review.
...
- The note accepts marking a workflow suggestion as verified because it is useful.
...
- The next review should verify whether verified count, advisory count, unknown count, and evidence gap still captures the decision quality.
