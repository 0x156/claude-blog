...
## Summary
FLOW Review Stage is the spoke for review draft quality, usefulness, voice, links, schema needs, and risk flags.
Use it when [[FLOW Framework]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a FLOW review packet.
The success condition is that review converts draft findings into concrete revision actions.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs FLOW Review Stage.
...
- Metric set for review: revision count, blocker count, reviewer notes, and confidence.

## Operating Procedure
- Start with the precise decision that FLOW Review Stage must make.
...
- Reject the failure pattern of reviewing only grammar while missing source and trust defects.
...
## Acceptance Criteria
- The FLOW review packet names the exact object under review.
...
- The note accepts reviewing only grammar while missing source and trust defects.
...
- The next review should verify whether revision count, blocker count, reviewer notes, and confidence still captures the decision quality.
