...
## Summary
FLOW Read Only Boundary is the spoke for define what the vault can advise and what it must not mutate in V1.
Use it when [[FLOW Framework]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a read only boundary note.
The success condition is that the brain gives recommendations without changing CMS, GSC, GA4, or publishing systems.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs FLOW Read Only Boundary.
...
- Metric set for review: external action count, approval need, blocked mutation, and handoff owner.

## Operating Procedure
- Start with the precise decision that FLOW Read Only Boundary must make.
...
- Reject the failure pattern of using an advisory note as permission to mutate external systems.
...
## Acceptance Criteria
- The read only boundary note names the exact object under review.
...
- The note accepts using an advisory note as permission to mutate external systems.
...
- The next review should verify whether external action count, approval need, blocked mutation, and handoff owner still captures the decision quality.
