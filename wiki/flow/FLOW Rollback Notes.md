...
## Summary
FLOW Rollback Notes is the spoke for attach rollback and review triggers to recommendations that affect live content.
Use it when [[FLOW Framework]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a FLOW rollback note.
The success condition is that every meaningful live content recommendation has a reversal path.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs FLOW Rollback Notes.
...
- Metric set for review: rollback trigger, owner, observation window, and review status.

## Operating Procedure
- Start with the precise decision that FLOW Rollback Notes must make.
...
- Reject the failure pattern of advisory recommendations becoming permanent without review.
...
## Acceptance Criteria
- The FLOW rollback note names the exact object under review.
...
- The note accepts advisory recommendations becoming permanent without review.
...
- The next review should verify whether rollback trigger, owner, observation window, and review status still captures the decision quality.
