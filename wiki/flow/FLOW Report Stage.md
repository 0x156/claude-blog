...
## Summary
FLOW Report Stage is the spoke for summarize decisions, evidence, scores, risks, and next actions for handoff.
Use it when [[FLOW Framework]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a FLOW report packet.
The success condition is that reports make recommendations auditable and bounded.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs FLOW Report Stage.
...
- Metric set for review: recommendation count, confidence mix, blockers, and next owner.

## Operating Procedure
- Start with the precise decision that FLOW Report Stage must make.
...
- Reject the failure pattern of reporting confident outcomes from incomplete evidence.
...
## Acceptance Criteria
- The FLOW report packet names the exact object under review.
...
- The note accepts reporting confident outcomes from incomplete evidence.
...
- The next review should verify whether recommendation count, confidence mix, blockers, and next owner still captures the decision quality.
