...
Sensitive topics need stronger reviewer involvement through [[E-E-A-T for Blog Content]]. A template can improve clarity, but it cannot compensate for weak evidence. When two templates fit, pick the one with the clearest completion test. If the selected format requires claims the source packet cannot support, change the format before drafting.
