...
## Summary
Market Average Versus First Party Data is a planning note for dual optimization.
It keeps market studies subordinate to Search Console, analytics, and property evidence.
Use it with [[Dual Optimization]] when the working unit is a blog program, article, or section.

## Operating Question
- When should the team replace a market study with first-party evidence?
- The expected output is a data hierarchy note with fallback rules.
- The main risk is using public studies when property data is already available.
...
## Specific Checks
- Check that market average versus first party data is applied to the correct content unit.
- Check that the decision matches this purpose: keeps market studies subordinate to Search Console, analytics, and property evidence.
- Check that the output can be inspected as a data hierarchy note with fallback rules.
- Check that the risk is addressed directly: using public studies when property data is already available.
