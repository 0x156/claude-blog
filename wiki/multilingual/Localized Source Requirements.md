...
## Summary
This spoke requires local or translated evidence when claims depend on country, law, pricing, culture, or availability.
...
Primary working inputs: claim list, local sources, source dates, reviewer comments.
...
## Workflow
- Define the decision this note supports: requires local or translated evidence when claims depend on country, law, pricing, culture, or availability.
...
## Risk Controls
- Evidence currency risk: Localized Source Requirements decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Localized Source Requirements advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Localized Source Requirements into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Localized Source Requirements guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Localized Source Requirements outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Localized Source Requirements decision summary for the editor or auditor.
...
- A blocked-claims list for Localized Source Requirements gaps that need more evidence.
...
- A no-action statement when Localized Source Requirements evidence is incomplete or outside this brain.
