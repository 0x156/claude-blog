...
## Summary
This spoke defines when the fallback language or selector URL should use x-default.
...
Primary working inputs: global selector URL, default language page, alternate set, canonical map.
...
## Workflow
- Define the decision this note supports: defines when the fallback language or selector URL should use x-default.
...
## Risk Controls
- Evidence currency risk: x-default Handling decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep x-default Handling advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn x-default Handling into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust x-default Handling guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in x-default Handling outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A x-default Handling decision summary for the editor or auditor.
...
- A blocked-claims list for x-default Handling gaps that need more evidence.
...
- A no-action statement when x-default Handling evidence is incomplete or outside this brain.
