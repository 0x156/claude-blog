...
## Summary
This spoke checks whether the target locale has different search intent, terminology, examples, or objections.
...
Primary working inputs: SERP notes, local queries, competing pages, regional modifiers.
...
## Workflow
- Define the decision this note supports: checks whether the target locale has different search intent, terminology, examples, or objections.
...
## Risk Controls
- Evidence currency risk: Locale Intent Research decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Locale Intent Research advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Locale Intent Research into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Locale Intent Research guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Locale Intent Research outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Locale Intent Research decision summary for the editor or auditor.
...
- A blocked-claims list for Locale Intent Research gaps that need more evidence.
...
- A no-action statement when Locale Intent Research evidence is incomplete or outside this brain.
