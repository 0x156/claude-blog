...
## Summary
Cluster Refresh Cadence is the spoke for define review timing for hubs, spokes, sources, links, and performance evidence.
...
The durable output is a refresh cadence calendar.
The success condition is that cluster maintenance follows source volatility and first-party performance data.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Cluster Refresh Cadence.
...
- Metric set for review: stale pages, update triggers, source age, and review completion.

## Operating Procedure
- Start with the precise decision that Cluster Refresh Cadence must make.
...
- Reject the failure pattern of refreshing every page on a calendar without evidence of need.
...
## Acceptance Criteria
- The refresh cadence calendar names the exact object under review.
...
- The note accepts refreshing every page on a calendar without evidence of need.
...
- The next review should verify whether stale pages, update triggers, source age, and review completion still captures the decision quality.
