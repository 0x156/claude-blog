...
## Summary
Cluster Hub Selection is the spoke for choose the canonical hub page that owns a broad topic, reader promise, and internal link center.
...
The durable output is a hub decision record.
The success condition is that one page owns the broad topic and all spokes justify a distinct reader job.
Keep the note advisory in V1 and route implementation to a human owner.

## Parent Hub Fit
- Parent hub: [[Semantic Topic Clusters]].
- Primary upstream context: [[Dual Optimization]].
- Primary downstream context: [[SERP-Informed Briefs and Outlines]].
- Evidence route: [[Research Pack Index]].
- Sibling comparison starts with [[Spoke Note Inventory]] and [[Entity Extraction Workflow]].
...
- Google Cloud Natural Language documentation, retrieved 2026-07-06, is the route for entity, salience, and Knowledge Graph mid extraction.
- Search Console Search Analytics API documentation, retrieved 2026-07-06, is the first-party route for clicks, impressions, CTR, and position by query and page.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Cluster Hub Selection.
- Parent hub context from [[Semantic Topic Clusters]].
...
- Metric set for review: hub impressions, spoke entrances, assisted internal clicks, and cannibalization warnings.

## Operating Procedure
- Start with the precise decision that Cluster Hub Selection must make.
- Restate the scope so it cannot drift beyond [[Semantic Topic Clusters]].
...
- Reject the failure pattern of making a hub because a keyword is broad while the site lacks original support evidence.
...
## Acceptance Criteria
- The hub decision record names the exact object under review.
...
- The note accepts making a hub because a keyword is broad while the site lacks original support evidence.
- The note hides unresolved uncertainty instead of naming it.
- The note duplicates another sibling without a clear boundary.

## Handoff
- Send brief structure questions to [[Spoke Note Inventory]].
- Send evidence and source issues to [[Entity Extraction Workflow]].
...
- The next review should verify whether hub impressions, spoke entrances, assisted internal clicks, and cannibalization warnings still captures the decision quality.
