...
## Summary
Spoke Note Inventory is the spoke for list existing, missing, stale, and consolidated support pages for one topic cluster.
...
The durable output is a spoke inventory table.
The success condition is that every spoke has a clear role, status, source need, and canonical target.
...
- Sibling comparison starts with [[Cluster Hub Selection]] and [[Entity Extraction Workflow]].
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Spoke Note Inventory.
...
- Metric set for review: coverage status, source readiness, refresh priority, and consolidation count.

## Operating Procedure
- Start with the precise decision that Spoke Note Inventory must make.
...
- Reject the failure pattern of treating an inventory as permission to publish thin pages.
...
## Acceptance Criteria
- The spoke inventory table names the exact object under review.
...
- The note accepts treating an inventory as permission to publish thin pages.
...
- The next review should verify whether coverage status, source readiness, refresh priority, and consolidation count still captures the decision quality.
