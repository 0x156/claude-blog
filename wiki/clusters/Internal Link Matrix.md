...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Internal Link Matrix.
...
- Metric set for review: link depth, anchor clarity, orphan status, and reciprocal coverage.

## Operating Procedure
- Start with the precise decision that Internal Link Matrix must make.
...
- Reject the failure pattern of adding links only for crawl paths without reader value.
...
## Acceptance Criteria
- The internal link matrix names the exact object under review.
...
- The note accepts adding links only for crawl paths without reader value.
...
- The next review should verify whether link depth, anchor clarity, orphan status, and reciprocal coverage still captures the decision quality.
