...
## Summary
Cluster Canonical Page Rules is the spoke for define when a hub, spoke, or older article should become the canonical owner for an intent.
...
The durable output is a canonical page decision rule.
The success condition is that canonical choices reduce duplication while preserving useful reader paths.
...
- Sibling comparison starts with [[Cluster Hub Selection]] and [[Spoke Note Inventory]].
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Cluster Canonical Page Rules.
...
- Metric set for review: canonical owner count, merge decisions, redirect candidates, and review dates.

## Operating Procedure
- Start with the precise decision that Cluster Canonical Page Rules must make.
...
- Reject the failure pattern of using canonical language to hide unresolved content overlap.
...
## Acceptance Criteria
- The canonical page decision rule names the exact object under review.
...
- The note accepts using canonical language to hide unresolved content overlap.
- The note hides unresolved uncertainty instead of naming it.
- The note duplicates another sibling without a clear boundary.

## Handoff
- Send brief structure questions to [[Cluster Hub Selection]].
- Send evidence and source issues to [[Spoke Note Inventory]].
...
- The next review should verify whether canonical owner count, merge decisions, redirect candidates, and review dates still captures the decision quality.
