...
## Summary
Cluster Gap Analysis is the spoke for identify missing support pages that would add original value to the hub.
...
The durable output is a gap analysis list.
The success condition is that gaps become briefs only when they have evidence, reader demand, and unique value.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Cluster Gap Analysis.
...
- Metric set for review: validated gaps, rejected gaps, evidence needs, and brief readiness.

## Operating Procedure
- Start with the precise decision that Cluster Gap Analysis must make.
...
- Reject the failure pattern of manufacturing spokes to fill a diagram.
...
## Acceptance Criteria
- The gap analysis list names the exact object under review.
...
- The note accepts manufacturing spokes to fill a diagram.
...
- The next review should verify whether validated gaps, rejected gaps, evidence needs, and brief readiness still captures the decision quality.
