...
## Summary
Evidence Density For Blog Posts is a drafting note for six-pillar writing.
It sets the minimum source and proof density for claim-heavy writing.
Use it with [[6-Pillar Dual Optimization]] when the working unit is a draft section, outline, or article.

## Operating Question
- Are claims close enough to evidence for a reviewer to verify them quickly?
- The expected output is an evidence-density note with claim counts, source dates, and gaps.
- The main risk is stacking claims in a paragraph without source proximity.
...
## Specific Checks
- Check that evidence density for blog posts is applied to the correct content unit.
- Check that the decision matches this purpose: sets the minimum source and proof density for claim-heavy writing.
- Check that the output can be inspected as an evidence-density note with claim counts, source dates, and gaps.
- Check that the risk is addressed directly: stacking claims in a paragraph without source proximity.
