...
## Summary
YMYL Adjacent Blog Policy is a trust review note for E-E-A-T review.
It sets stricter rules for content near sensitive decisions even when it is not formally regulated.
Use it with [[E-E-A-T for Blog Content]] when the working unit is a author, page, source set, or editorial process.

## Operating Question
- What extra caution is needed when a blog topic touches money, health, safety, civic life, or legal choices?
- The expected output is a YMYL-adjacent policy note with escalation triggers.
- The main risk is avoiding escalation because the post is framed as general advice.
...
## Specific Checks
- Check that ymyl adjacent blog policy is applied to the correct content unit.
- Check that the decision matches this purpose: sets stricter rules for content near sensitive decisions even when it is not formally regulated.
- Check that the output can be inspected as a YMYL-adjacent policy note with escalation triggers.
- Check that the risk is addressed directly: avoiding escalation because the post is framed as general advice.
