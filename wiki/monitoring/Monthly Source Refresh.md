...
## Summary
This spoke sets the monthly routine for Search Central, status dashboard, QRG, schema, and AI search sources.
...
Primary working inputs: source list, retrieval date, owner, next due date.
...
## Workflow
- Define the decision this note supports: sets the monthly routine for Search Central, status dashboard, QRG, schema, and AI search sources.
...
## Risk Controls
- Evidence currency risk: Monthly Source Refresh decisions can go stale when a source, date, or requirement changes.
- Claim scope risk: keep Monthly Source Refresh advice tied to the source coverage named in this note.
- Operational boundary risk: do not turn Monthly Source Refresh into CMS, analytics, Search Console, API, or publishing mutation steps.
- Proof gap risk: when the note lacks direct evidence, record the gap instead of upgrading confidence.
- Audience fit risk: adjust Monthly Source Refresh guidance for topic sensitivity, locale, and review ownership before use.
- Media and data risk: verify charts, visuals, metrics, and examples before they carry this note's claims.
- Metric interpretation risk: separate first-party exports, market studies, and API signals in Monthly Source Refresh outputs.
- Monitoring risk: quarantine unconfirmed volatility until a dated source in the ledger supports it.

## Output Shape
- A Monthly Source Refresh decision summary for the editor or auditor.
...
- A blocked-claims list for Monthly Source Refresh gaps that need more evidence.
...
- A no-action statement when Monthly Source Refresh evidence is incomplete or outside this brain.
