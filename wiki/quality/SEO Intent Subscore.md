...
## Summary
SEO Intent Subscore is the spoke for score query fit, intent alignment, title promise, internal links, and metadata discipline.
Use it when [[Blog Quality Score]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a SEO intent subscore.
The success condition is that SEO checks support reader fit without treating position as guaranteed.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs SEO Intent Subscore.
...
- Metric set for review: intent fit, title match, metadata clarity, and internal link coverage.

## Operating Procedure
- Start with the precise decision that SEO Intent Subscore must make.
...
- Reject the failure pattern of optimizing for a keyword that the article does not satisfy.
...
## Acceptance Criteria
- The SEO intent subscore names the exact object under review.
...
- The note accepts optimizing for a keyword that the article does not satisfy.
...
- The next review should verify whether intent fit, title match, metadata clarity, and internal link coverage still captures the decision quality.
