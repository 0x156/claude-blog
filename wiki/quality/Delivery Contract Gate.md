...
## Summary
Delivery Contract Gate is the spoke for decide whether a blog draft is ready for delivery, revision, or blocked status.
Use it when [[Blog Quality Score]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a delivery gate report.
The success condition is that handoff status reflects quality, source, risk, schema, and citation checks.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Delivery Contract Gate.
...
- Metric set for review: ready status, revision items, blocker count, and delivery confidence.

## Operating Procedure
- Start with the precise decision that Delivery Contract Gate must make.
...
- Reject the failure pattern of shipping a draft with hidden critical issues.
...
## Acceptance Criteria
- The delivery gate report names the exact object under review.
...
- The note accepts shipping a draft with hidden critical issues.
...
- The next review should verify whether ready status, revision items, blocker count, and delivery confidence still captures the decision quality.
