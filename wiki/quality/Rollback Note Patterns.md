...
## Summary
Rollback Note Patterns is the spoke for write rollback notes for content changes that could affect visibility or trust.
Use it when [[Blog Quality Score]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a rollback note pattern library.
The success condition is that recommendations include a way to reverse or review the change.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Rollback Note Patterns.
...
- Metric set for review: rollback completeness, trigger clarity, owner, and review date.

## Operating Procedure
- Start with the precise decision that Rollback Note Patterns must make.
...
- Reject the failure pattern of making irreversible live edits from an advisory vault.
...
## Acceptance Criteria
- The rollback note pattern library names the exact object under review.
...
- The note accepts making irreversible live edits from an advisory vault.
...
- The next review should verify whether rollback completeness, trigger clarity, owner, and review date still captures the decision quality.
