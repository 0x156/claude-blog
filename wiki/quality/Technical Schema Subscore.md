...
## Summary
Technical Schema Subscore is the spoke for score indexability basics, mobile parity, structured data, Core Web Vitals, and media hygiene.
Use it when [[Blog Quality Score]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a technical schema subscore.
The success condition is that technical points reflect current Google and web.dev requirements.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Technical Schema Subscore.
...
- Metric set for review: schema validity, Article priority, CWV status, and mobile parity.

## Operating Procedure
- Start with the precise decision that Technical Schema Subscore must make.
...
- Reject the failure pattern of rewarding deprecated FAQ rich result expectations.
...
## Acceptance Criteria
- The technical schema subscore names the exact object under review.
...
- The note accepts rewarding deprecated FAQ rich result expectations.
...
- The next review should verify whether schema validity, Article priority, CWV status, and mobile parity still captures the decision quality.
