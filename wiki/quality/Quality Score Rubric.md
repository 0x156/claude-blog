...
## Summary
Quality Score Rubric is the spoke for define the five-category 100-point review model and its scoring thresholds.
Use it when [[Blog Quality Score]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a quality score rubric.
The success condition is that scores are inspectable, source-aware, and advisory.
...
- Sibling comparison starts with [[Content Quality Subscore]] and [[SEO Intent Subscore]].
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Quality Score Rubric.
...
- Metric set for review: total score, category score, block count, and confidence.

## Operating Procedure
- Start with the precise decision that Quality Score Rubric must make.
...
- Reject the failure pattern of using a score to guarantee rankings, traffic, or AI citations.
...
## Acceptance Criteria
- The quality score rubric names the exact object under review.
...
- The note accepts using a score to guarantee rankings, traffic, or AI citations.
- The note hides unresolved uncertainty instead of naming it.
- The note duplicates another sibling without a clear boundary.

## Handoff
- Send brief structure questions to [[Content Quality Subscore]].
- Send evidence and source issues to [[SEO Intent Subscore]].
...
- The next review should verify whether total score, category score, block count, and confidence still captures the decision quality.
