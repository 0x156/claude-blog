...
## Summary
Quality Review Evidence Log is the spoke for capture the exact evidence used during a score review.
Use it when [[Blog Quality Score]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a quality evidence log.
The success condition is that the score can be audited after handoff.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Quality Review Evidence Log.
...
- Metric set for review: evidence rows, source dates, reviewer notes, and unresolved claims.

## Operating Procedure
- Start with the precise decision that Quality Review Evidence Log must make.
...
- Reject the failure pattern of scoring from memory or preference without source references.
...
## Acceptance Criteria
- The quality evidence log names the exact object under review.
...
- The note accepts scoring from memory or preference without source references.
...
- The next review should verify whether evidence rows, source dates, reviewer notes, and unresolved claims still captures the decision quality.
