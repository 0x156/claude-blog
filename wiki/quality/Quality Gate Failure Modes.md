...
## Summary
Quality Gate Failure Modes is the spoke for define blockers, warnings, and advisory issues that stop or slow publication.
Use it when [[Blog Quality Score]] needs a repeatable decision record before a brief, draft, score, or report moves forward.
The durable output is a failure mode register.
The success condition is that serious source, trust, spam, or schema defects are visible before handoff.
...
## Required Inputs
- Target topic, article, cluster, or workflow item that needs Quality Gate Failure Modes.
...
- Metric set for review: blockers, warnings, unresolved unknowns, and owner assignments.

## Operating Procedure
- Start with the precise decision that Quality Gate Failure Modes must make.
...
- Reject the failure pattern of letting a high total score hide a blocking defect.
...
## Acceptance Criteria
- The failure mode register names the exact object under review.
...
- The note accepts letting a high total score hide a blocking defect.
...
- The next review should verify whether blockers, warnings, unresolved unknowns, and owner assignments still captures the decision quality.
